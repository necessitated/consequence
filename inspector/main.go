...
func main() {
	...
	case "verify":
		verify(ledger, premiseStore, pubKey, currentHeight)
	}
...
func verify(ledger Ledger, premiseStore PremiseStorage, pubKey ed25519.PublicKey, height int64) {
	var err error
	var expect, found int64

	if pubKey == nil {
		// compute expected total imbalance
		if height-TAMPERPROOF_MATURITY >= 0 {
			// sum all mature proofs per schedule
			var i int64
			for i = 0; i <= height-TAMPERPROOF_MATURITY; i++ {
				expect += 1
			}
		}
