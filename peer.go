...
type Peer struct {
	...
	outbound                      bool
	localDownloadQueue            *StageQueue // peer-local download queue
	localInflightQueue            *StageQueue // peer-local inflight queue
	globalInflightQueue           *StageQueue // global inflight queue
	ignoreStages                  map[StageID]bool
	continuationStageID           StageID
...
func NewPeer(conn *websocket.Conn, genesisID StageID, peerStore PeerStorage,
	stageStore StageStorage, ledger Ledger, processor *Processor, indexer *Indexer,
	txQueue TransitionQueue, stageQueue *StageQueue, addrChan chan<- string) *Peer {
	peer := &Peer{
		conn:                conn,
		genesisID:           genesisID,
		peerStore:           peerStore,
		...
		indexer:             indexer,
		txQueue:             txQueue,
		localDownloadQueue:  NewStageQueue(),
		localInflightQueue:  NewStageQueue(),
		globalInflightQueue: stageQueue,
		ignoreStages:        make(map[StageID]bool),
		addrChan:            addrChan,
	}
...
const (
	...
	// Time allowed between processing new stages before we consider a consequence sync stalled
	syncWait = 2 * time.Minute

	// Maximum stages per inv_stage message
	maxStagesPerInv = 500
	...
	downloadQueueMax = maxStagesPerInv * 10
)
...
func (p *Peer) run() {
	...
	defer func() {
		// remove any inflight stages this peer is no longer going to download
		stageInflight, ok := p.localInflightQueue.Peek()
		for ok {
			p.localInflightQueue.Remove(stageInflight, "")
			p.globalInflightQueue.Remove(stageInflight, peerAddr)
			stageInflight, ok = p.localInflightQueue.Peek()
		}
	}()
	...
	// written to by the reader loop to update the current work stage for the peer
	...
	go func() {
		...
		// register to hear about tip stage changes
		...
		// register to hear about new transitions
		newTxChan := make(chan NewTx, MAX_TRANSITIONS_TO_INCLUDE_PER_STAGE)
		p.processor.RegisterForNewTransitions(newTxChan)
		defer p.processor.UnregisterForNewTransitions(newTxChan)
		...
		for {
			...
			case tip := <-tipChangeChan:
				...
				if tip.Connect && tip.More == false {
					...
					p.createNewWorkStage(tip.StageID, tip.Stage.Header)
				}

				if tip.Source == p.conn.RemoteAddr().String() {
					// this is who sent us the stage that caused the change
					break
				}
				...
				if tip.Connect {
					...
					inv := Message{
						Type: "inv_stage",
						Body: InvStageMessage{
							StageIDs: []StageID{tip.StageID},
						},
					}
					// send it
					p.conn.SetWriteDeadline(time.Now().Add(writeWait))
				...
				// potentially create a filter_stage
				fb, err := p.createFilterStage(tip.StageID, tip.Stage)
				if err != nil {
				...
				if !tip.Connect {
					m.Type = "filter_stage_undo"
				}

				log.Printf("Sending %s with %d transition(s), to: %s\n",
					m.Type, len(fb.Transitions), p.conn.RemoteAddr())
				p.conn.SetWriteDeadline(time.Now().Add(writeWait))
				if err := p.conn.WriteJSON(m); err != nil {
					log.Printf("Write error: %s, to: %s\n", err, p.conn.RemoteAddr())
					p.conn.Close()
			...
			case newTx := <-newTxChan:
				...
				interested := func() bool {
					...
					return p.filterLookup(newTx.Transition)
				}()
				if !interested {
					continue
				}

				// newly verified transition announced, relay to peer
				pushTx := Message{
					Type: "push_transition",
					Body: PushTransitionMessage{
						Transition: newTx.Transition,
					},
				}
				p.conn.SetWriteDeadline(time.Now().Add(writeWait))
			...
			case <-tickerUpdateWorkCheck.C:
				...
				txCount := len(p.workStage.Transitions)
				if txCount == MAX_TRANSITIONS_TO_INCLUDE_PER_STAGE {
					// already at capacity
					break
				}
				if txCount-1 != p.txQueue.Len() {
					tipID, tipHeader, _, err := getSequenceTipHeader(p.ledger, p.stageStore)
					if err != nil {
						log.Printf("Error reading sequence tip header: %s\n", err)
						break
					}
					p.createNewWorkStage(*tipID, tipHeader)
				}
			}
		}
	}()
	...
	// are we syncing?
	lastNewStageTime := time.Now()
	ibd, _, err := IsInitialStageDownload(p.ledger, p.stageStore)
	if err != nil {
		log.Println(err)
		return
	}
	...
	p.conn.SetPongHandler(func(string) error {
		if ibd {
			...
			if ibd && time.Since(lastNewStageTime) > syncWait {
				return fmt.Errorf("Sync has stalled, disconnecting")
			}
		} else {
			// try processing the queue in case we've been stageed by another client
	...
	for {
		...
		case websocket.TextMessage:
			...
			if m.Type != "stage" && len(message) > MAX_PROTOCOL_MESSAGE_LENGTH {
			...
			switch m.Type {
			case "inv_stage":
				var inv InvStageMessage
				if err := json.Unmarshal(body, &inv); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					return
				}
				for i, id := range inv.StageIDs {
					if err := p.onInvStage(id, i, len(inv.StageIDs), outChan); err != nil {
						log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
						break
					}
				}
			...
			case "get_stage":
				var gb GetStageMessage
				if err := json.Unmarshal(body, &gb); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					return
				}
				if err := p.onGetStage(gb.StageID, outChan); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					break
				}

			case "get_stage_by_height":
				var gbbh GetStageByHeightMessage
				if err := json.Unmarshal(body, &gbbh); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					return
				}
				if err := p.onGetStageByHeight(gbbh.Height, outChan); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					break
				}
			...
			case "stage":
				...
				if b.Stage == nil {
					log.Printf("Error: received nil stage, from: %s\n", p.conn.RemoteAddr())
					return
				}
				if b.Stage.Header == nil {
					log.Printf("Error: received nil stage header, from: %s\n", p.conn.RemoteAddr())
					return
				}
				ok, err := p.onStage(b.Stage, ibd, outChan)
				if err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					break
				}
			...
			case "find_common_ancestor":
				...
				num := len(fca.StageIDs)
				for i, id := range fca.StageIDs {
			...
			case "get_stage_header":
				var gbh GetStageHeaderMessage
				if err := json.Unmarshal(body, &gbh); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					return
				}
				if err := p.onGetStageHeader(gbh.StageID, outChan); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					break
				}

			case "get_stage_header_by_height":
				var gbhbh GetStageHeaderByHeightMessage
				if err := json.Unmarshal(body, &gbhbh); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					return
				}
				if err := p.onGetStageHeaderByHeight(gbhbh.Height, outChan); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					break
				}
			...
			case "get_public_key_transitions":
				var gpkt GetPublicKeyTransitionsMessage
				if err := json.Unmarshal(body, &gpkt); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					return
				}
				if err := p.onGetPublicKeyTransitions(gpkt.PublicKey,
			...
			case "get_transition":
				var gt GetTransitionMessage
				if err := json.Unmarshal(body, &gt); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					return
				}
				if err := p.onGetTransition(gt.TransitionID, outChan); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					break
				}
			...
			case "push_transition":
				var pt PushTransitionMessage
				if err := json.Unmarshal(body, &pt); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					return
				}
				if pt.Transition == nil {
					log.Printf("Error: received nil transition, from: %s\n", p.conn.RemoteAddr())
					return
				}
				if err := p.onPushTransition(pt.Transition, outChan); err != nil {
					log.Printf("Error: %s, from: %s\n", err, p.conn.RemoteAddr())
					break
				}

			case "push_transition_result":
				var ptr PushTransitionResultMessage
			...
			case "get_filter_transition_queue":
				p.onGetFilterTransitionQueue(outChan)
...
// Handle a message from a peer indicating stage inventory available for download
func (p *Peer) onInvStage(id StageID, index, length int, outChan chan<- Message) error {
	log.Printf("Received inv_stage: %s, from: %s\n", id, p.conn.RemoteAddr())

	if length > maxStagesPerInv {
		return fmt.Errorf("%d stages IDs is more than %d maximum per inv_stage",
			length, maxStagesPerInv)
	}

	// is it on the ignore list?
	if p.ignoreStages[id] {
		log.Printf("Ignoring stage %s, from: %s\n", id, p.conn.RemoteAddr())
		return nil
	}

	// do we have it queued or inflight already?
	if p.localDownloadQueue.Exists(id) || p.localInflightQueue.Exists(id) {
		log.Printf("Stage %s is already queued or inflight for download, from: %s\n",
			id, p.conn.RemoteAddr())
		return nil
	}
	...
	if branchType != UNKNOWN {
		...
		if length > 1 && index+1 == length {
			// we might be on a deep side sequence. this will get us the next 500 stages
...
// Handle a request for a stage from a peer
func (p *Peer) onGetStage(id StageID, outChan chan<- Message) error {
	log.Printf("Received get_stage: %s, from: %s\n", id, p.conn.RemoteAddr())
	return p.getStage(id, outChan)
}

// Handle a request for a stage by height from a peer
func (p *Peer) onGetStageByHeight(height int64, outChan chan<- Message) error {
	log.Printf("Received get_stage_by_height: %d, from: %s\n", height, p.conn.RemoteAddr())
	id, err := p.ledger.GetStageIDForHeight(height)
	if err != nil {
		// not found
		outChan <- Message{Type: "stage"}
		return err
	}
	if id == nil {
		// not found
		outChan <- Message{Type: "stage"}
		return fmt.Errorf("No stage found at height %d", height)
	}
	return p.getStage(*id, outChan)
}

func (p *Peer) getStage(id StageID, outChan chan<- Message) error {
	// fetch the stage
	stageJson, err := p.stageStore.GetStageBytes(id)
	if err != nil {
		// not found
		outChan <- Message{Type: "stage", Body: StageMessage{StageID: &id}}
		return err
	}
	if len(stageJson) == 0 {
		// not found
		outChan <- Message{Type: "stage", Body: StageMessage{StageID: &id}}
		return fmt.Errorf("No stage found with ID %s", id)
	}

	// send out the raw bytes
	body := []byte(`{"stage_id":"`)
	body = append(body, []byte(id.String())...)
	body = append(body, []byte(`","stage":`)...)
	body = append(body, stageJson...)
	body = append(body, []byte(`}`)...)
	outChan <- Message{Type: "stage", Body: json.RawMessage(body)}

	// was this the last stage in the inv we sent in response to a find common ancestor request?
	if id == p.continuationStageID {
		log.Printf("Received get_stage for continuation stage %s, from: %s\n",
			id, p.conn.RemoteAddr())
		p.continuationStageID = StageID{}
		// send an inv for our tip stage to prompt the peer to
		...
		if tipID != nil {
			outChan <- Message{Type: "inv_stage", Body: InvStageMessage{StageIDs: []StageID{*tipID}}}
		}
	}
	return nil
}

// Handle receiving a stage from a peer. Returns true if the stage was newly processed and accepted.
func (p *Peer) onStage(stage *Stage, ibd bool, outChan chan<- Message) (bool, error) {
	...
	log.Printf("Received stage: %s, from: %s\n", id, p.conn.RemoteAddr())

	stageInFlight, ok := p.localInflightQueue.Peek()
	if !ok || stageInFlight != id {
		// disconnect misbehaving peer
		p.conn.Close()
		return false, fmt.Errorf("Received unrequested stage")
	}

	// don't process low difficulty stages
	if ibd == false && CheckpointsEnabled && stage.Header.Height < LatestCheckpointHeight {
		...
		// ignore future inv_stages for this stage
		p.ignoreStages[id] = true
		if len(p.ignoreStages) > maxStagesPerInv {
			// they're intentionally sending us bad stages
			log.Printf("Disconnecting %s, max ignore list size exceeded\n", p.conn.RemoteAddr().String())
			p.conn.Close()
		}
		return false, fmt.Errorf("Stage %s height %d less than latest checkpoint height %d",
			id, stage.Header.Height, LatestCheckpointHeight)
	}

	var accepted bool

	// is it an orphan?
	header, _, err := p.stageStore.GetStageHeader(stage.Header.Previous)
	if err != nil || header == nil {
		...
		log.Printf("Stage %s is an orphan, sending find_common_ancestor to: %s\n",
			id, p.conn.RemoteAddr())
		...
		// process the stage
		if err := p.processor.ProcessStage(id, stage, p.conn.RemoteAddr().String()); err != nil {
			// disconnect a peer that sends us a bad stage
			p.conn.Close()
			return false, err
		}
		// newly accepted stage
		accepted = true
	...
	// see if there are any more stages to download right now
	if err := p.processDownloadQueue(outChan); err != nil {
		return false, err
	}
...
// Try requesting stages that are in the download queue
func (p *Peer) processDownloadQueue(outChan chan<- Message) error {
	...
	for p.localInflightQueue.Len() < inflightQueueMax {
		// next stage to download
		stageToDownload, ok := p.localDownloadQueue.Peek()
		if !ok {
			// no more stages in the queue
			break
		}

		// double-check if it's been processed since we last checked
		branchType, err := p.ledger.GetBranchType(stageToDownload)
		if err != nil {
			return err
		}
		if branchType != UNKNOWN {
			// it's been processed. remove it and check the next one
			log.Printf("Stage %s has been processed, removing from download queue for: %s\n",
				stageToDownload, p.conn.RemoteAddr().String())
			p.localDownloadQueue.Remove(stageToDownload, "")
			continue
		}

		// add stage to the global inflight queue with this peer as the owner
		if p.globalInflightQueue.Add(stageToDownload, p.conn.RemoteAddr().String()) == false {
			// another peer is downloading it right now.
			// wait to see if they succeed before trying to download any others
			log.Printf("Stage %s is being downloaded already from another peer\n", stageToDownload)
			break
		}
		...
		p.localInflightQueue.Add(stageToDownload, "")
		queued++

		// request it
		log.Printf("Sending get_stage for %s, to: %s\n", stageToDownload, p.conn.RemoteAddr())
		outChan <- Message{Type: "get_stage", Body: GetStageMessage{StageID: stageToDownload}}
	}

	if queued > 0 {
		log.Printf("Requested %d stage(s) for download, from: %s", queued, p.conn.RemoteAddr())
...
func (p *Peer) sendFindCommonAncestor(startID *StageID, writeNow bool, outChan chan<- Message) error {
	...
	if startID == nil {
		...
		header, _, err := p.stageStore.GetStageHeader(*startID)
		if err != nil {
			return err
		}
		if header == nil {
			return fmt.Errorf("No header for stage %s", *startID)
		}
	...
	for id != nil {
		...
		var err error
		id, err = p.ledger.GetStageIDForHeight(depth)
		if err != nil {
			log.Printf("Error: %s\n", err)
			return nil
		}
	...
	m := Message{Type: "find_common_ancestor", Body: FindCommonAncestorMessage{StageIDs: ids}}
...
func (p *Peer) onFindCommonAncestor(id StageID, index, length int, outChan chan<- Message) (bool, error) {
	log.Printf("Received find_common_ancestor: %s, index: %d, length: %d, from: %s\n",
		id, index, length, p.conn.RemoteAddr())

	header, _, err := p.stageStore.GetStageHeader(id)
	if err != nil {
		return false, err
	}
	if header == nil {
	...
	for len(ids) < maxStagesPerInv {
		nextID, err := p.ledger.GetStageIDForHeight(height)
		if err != nil {
			return false, err
		}
		if nextID == nil {
		...
		log.Printf("Queueing inv for stage %s, height: %d, to: %s\n",
	...
	if len(ids) > 0 {
		...
		p.continuationStageID = ids[len(ids)-1]
		log.Printf("Sending inv_stage with %d IDs, continuation stage: %s, to: %s",
			len(ids), p.continuationStageID, p.conn.RemoteAddr())
		outChan <- Message{Type: "inv_stage", Body: InvStageMessage{StageIDs: ids}}
	}
	return true, nil
}

// Handle a request for a stage header from a peer
func (p *Peer) onGetStageHeader(id StageID, outChan chan<- Message) error {
	log.Printf("Received get_stage_header: %s, from: %s\n", id, p.conn.RemoteAddr())
	return p.getStageHeader(id, outChan)
}

// Handle a request for a stage header by ID from a peer
func (p *Peer) onGetStageHeaderByHeight(height int64, outChan chan<- Message) error {
	log.Printf("Received get_stage_header_by_height: %d, from: %s\n", height, p.conn.RemoteAddr())
	id, err := p.ledger.GetStageIDForHeight(height)
	if err != nil {
		// not found
		outChan <- Message{Type: "stage_header"}
		return err
	}
	if id == nil {
		// not found
...
func (p *Peer) getStageHeader(id StageID, outChan chan<- Message) error {
	header, _, err := p.stageStore.GetStageHeader(id)
	if err != nil {
		// not found
		outChan <- Message{Type: "stage_header", Body: StageHeaderMessage{StageID: &id}}
		return err
	}
	if header == nil {
		// not found
		outChan <- Message{Type: "stage_header", Body: StageHeaderMessage{StageID: &id}}
		return fmt.Errorf("Stage header for %s not found", id)
	}
	outChan <- Message{Type: "stage_header", Body: StageHeaderMessage{StageID: &id, StageHeader: header}}
	return nil
}
	...
	if ok {
		...
		outChan <- Message{
			...
			Body: ProfileMessage{
				...
				Bio: 	   bio,
				Locale:    locale,
				StageID:    p.indexer.latestStageID,
...
func (p *Peer) onGetImbalances(pubKeys []ed25519.PublicKey, outChan chan<- Message) error {
	...
	bm := ImbalancesMessage{StageID: tipID, Height: tipHeight}
...
// Handle a request for a public key's transitions over a given height range
func (p *Peer) onGetPublicKeyTransitions(pubKey ed25519.PublicKey,
	startHeight, endHeight int64, startIndex, limit int, outChan chan<- Message) error {
	log.Printf("Received get_public_key_transitions from: %s\n", p.conn.RemoteAddr())

	if limit < 0 {
		outChan <- Message{Type: "public_key_transitions"}
		return nil
	}
	...
	// get the indices for all transitions for the given public key
	// over the given range of stage heights
	bIDs, indices, stopHeight, stopIndex, err := p.ledger.GetPublicKeyTransitionIndicesRange(
		pubKey, startHeight, endHeight, startIndex, limit)
	if err != nil {
		outChan <- Message{Type: "public_key_transitions", Body: PublicKeyTransitionsMessage{Error: err.Error()}}
		return err
	}

	// build filter stages from the indices
	var fbs []*FilterStageMessage
	for i, stageID := range bIDs {
		// fetch transition and header
		tx, stageHeader, err := p.stageStore.GetTransition(stageID, indices[i])
		if err != nil {
			// odd case. just log it and continue
			log.Printf("Error retrieving transition history, stage: %s, index: %d, error: %s\n",
				stageID, indices[i], err)
			continue
		}
		// figure out where to put it
		var fb *FilterStageMessage
		if len(fbs) == 0 {
			// new stage
			fb = &FilterStageMessage{StageID: stageID, Header: stageHeader}
			fbs = append(fbs, fb)
		} else if fbs[len(fbs)-1].StageID != stageID {
			// new stage
			fb = &FilterStageMessage{StageID: stageID, Header: stageHeader}
			fbs = append(fbs, fb)
		} else {
			// transition is from the same stage
			fb = fbs[len(fbs)-1]
		}
		fb.Transitions = append(fb.Transitions, tx)
	}

	// send it to the writer
	outChan <- Message{
		Type: "public_key_transitions",
		Body: PublicKeyTransitionsMessage{
			PublicKey:    pubKey,
...
// Handle a request for a transition
func (p *Peer) onGetTransition(txID TransitionID, outChan chan<- Message) error {
	log.Printf("Received get_transition for %s, from: %s\n",
		txID, p.conn.RemoteAddr())

	stageID, index, err := p.ledger.GetTransitionIndex(txID)
	if err != nil {
		// not found
		outChan <- Message{Type: "transition", Body: TransitionMessage{TransitionID: txID}}
		return err
	}
	if stageID == nil {
		// not found
		outChan <- Message{Type: "transition", Body: TransitionMessage{TransitionID: txID}}
		return fmt.Errorf("Transition %s not found", txID)
	}
	tx, header, err := p.stageStore.GetTransition(*stageID, index)
	if err != nil {
		// odd case but send back what we know at least
		outChan <- Message{Type: "transition", Body: TransitionMessage{StageID: stageID, TransitionID: txID}}
		return err
	}
	if tx == nil {
		// another odd case
		outChan <- Message{
			Type: "transition",
			Body: TransitionMessage{
				StageID:       stageID,
				Height:        header.Height,
				TransitionID: txID,
			},
		}
		return fmt.Errorf("Transition at stage %s, index %d not found",
			*stageID, index)
	}
...
// Handle a request for a stage header of the tip of the main sequence from a peer
...
// Handle receiving a transition from a peer
func (p *Peer) onPushTransition(tx *Transition, outChan chan<- Message) error {
	id, err := tx.ID()
	if err != nil {
		outChan <- Message{Type: "push_transition_result", Body: PushTransitionResultMessage{Error: err.Error()}}
		return err
	}

	log.Printf("Received push_transition: %s, from: %s\n", id, p.conn.RemoteAddr())

	// process the transition if this is the first time we've seen it
	var errStr string
	if !p.txQueue.Exists(id) {
		err = p.processor.ProcessTransition(id, tx, p.conn.RemoteAddr().String())
		if err != nil {
			errStr = err.Error()
		}
	}

	outChan <- Message{Type: "push_transition_result",
		Body: PushTransitionResultMessage{
			TransitionID: id,
			Error:         errStr,
		},
	}
...
// Handle a request to set a transition filter for the connection
...
// Send back a filtered view of the transition queue
func (p *Peer) onGetFilterTransitionQueue(outChan chan<- Message) {
	log.Printf("Received get_filter_transition_queue, from: %s\n", p.conn.RemoteAddr())

	ftq := FilterTransitionQueueMessage{}
	...
	if p.filter == nil {
		...
		transitions := p.txQueue.Get(0)
		for _, tx := range transitions {
			if p.filterLookup(tx) {
				ftq.Transitions = append(ftq.Transitions, tx)
			}
		}
	}

	outChan <- Message{Type: "filter_transition_queue", Body: ftq}
}

// Returns true if the transition is of interest to the peer
func (p *Peer) filterLookup(tx *Transition) bool {
	if p.filter == nil {
		return true
	}
...
func (p *Peer) createFilterStage(id StageID, stage *Stage) (*FilterStageMessage, error) {
	p.filterLock.RLock()
	defer p.filterLock.RUnlock()
	...
	fb := FilterStageMessage{StageID: id, Header: stage.Header}

	// filter out transitions the peer isn't interested in
...
// Create a new work stage for a rendering peer. Called from the writer goroutine loop.
func (p *Peer) createNewWorkStage(tipID StageID, tipHeader *StageHeader) error {
	if len(p.pubKeys) == 0 {
		// peer doesn't want work
		return nil
	}

	medianTimestamp, err := computeMedianTimestamp(tipHeader, p.stageStore)
	if err != nil {
		...
		p.workID = rand.Int31()
		p.workStage, err = createNextStage(tipID, tipHeader, p.txQueue, p.stageStore, p.ledger, p.pubKeys[keyIndex], p.memo)
		if err != nil {
			log.Printf("Error creating next stage: %s, for: %s\n", err, p.conn.RemoteAddr())
		}
	}
	...
	if err != nil {
		...
		m.Body = WorkMessage{WorkID: p.workID, Header: p.workStage.Header, MinTime: p.medianTimestamp + 1}
	}
...
func (p *Peer) onSubmitWork(sw SubmitWorkMessage) {
	...
	if err != nil {
		log.Printf("Error computing stage ID: %s, from: %s\n", err, p.conn.RemoteAddr())
	} else if sw.WorkID == 0 {
		...
		p.workStage.Header = sw.Header
		err = p.processor.ProcessStage(id, p.workStage, p.conn.RemoteAddr().String())
		if err != nil {
			log.Printf("Error processing work stage: %s, from: %s\n", err, p.conn.RemoteAddr())
		}
	}
...
func (p *Peer) updateReadLimit() {
	ok, height, err := IsInitialStageDownload(p.ledger, p.stageStore)
	if err != nil {
		log.Fatal(err)
	}
	...
	// transitions are <500 bytes so this gives us significant wiggle room
	maxTransitions := computeMaxTransitionsPerStage(height + 1)
	p.readLimit = int64(maxTransitions) * 1024
}
