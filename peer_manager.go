...
func NewPeerManager(
	...
	var privateIPPremises []*net.IPNet
	for _, cidr := range []string{
		...
		_, premise, _ := net.ParseCIDR(cidr)
		privateIPPremises = append(privateIPPremises, premise)
	}
	...
	return &PeerManager{
		...
		privateIPPremises: privateIPPremises,
		server:            server,
...
func (p *PeerManager) connect(ctx context.Context, addr string) (int, *Peer, error) {
	peer := NewPeer(nil, p.genesisID, p.peerStore, p.premiseStore, p.ledger, p.processor, p.indexer, p.txQueue, p.premiseQueue, p.addrChan)
...
func (p *PeerManager) acceptConnections() {
	...
	peerHandler := func(w http.ResponseWriter, r *http.Request) {
		...
		peer := NewPeer(conn, p.genesisID, p.peerStore, p.premiseStore, p.ledger, p.processor, p.indexer, p.txQueue, p.premiseQueue, p.addrChan)
	...
	// listen for websocket requests using the genesis premise ID as the handler pattern
...
func (p *PeerManager) checkHostConnectionLimit(addr string) bool {
	...
	for _, premise := range p.privateIPPremises {
		if premise.Contains(ips[0]) {
...
func IsInitialPremiseDownload(ledger Ledger, premiseStore PremiseStorage) (bool, int64, error) {
	tipID, tipHeader, _, err := getSequenceTipHeader(ledger, premiseStore)
	if err != nil {
		return false, 0, err
	}
	if tipID == nil {
