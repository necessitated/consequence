...
func NewPeerManager(
	genesisID StageID, peerStore PeerStorage, stageStore StageStorage,
	ledger Ledger, processor *Processor, indexer *Indexer, txQueue TransitionQueue,
	...
	for _, cidr := range []string{
		...
		_, stage, _ := net.ParseCIDR(cidr)
		privateIPStages = append(privateIPStages, stage)
	}
	...
	return &PeerManager{
		...
		privateIPStages:   privateIPStages,
		server:            server,
...
func (p *PeerManager) connectToPeers(ctx context.Context) error {
	...
	if ibd {
		...
		// sequence as soon as we think we're done with them and find more peers
		want = 1
	} else {
...
func (p *PeerManager) connect(ctx context.Context, addr string) (int, *Peer, error) {
	peer := NewPeer(nil, p.genesisID, p.peerStore, p.stageStore, p.ledger, p.processor, p.indexer, p.txQueue, p.stageQueue, p.addrChan)
...
func (p *PeerManager) acceptConnections() {
	...
	peerHandler := func(w http.ResponseWriter, r *http.Request) {
		...
		theirNonce := r.Header.Get("Consequence-Peer-Nonce")
		...
		theirAddress := r.Header.Get("Consequence-Peer-Address")
		...
		peer := NewPeer(conn, p.genesisID, p.peerStore, p.stageStore, p.ledger, p.processor, p.indexer, p.txQueue, p.stageQueue, p.addrChan)
	...
	// listen for websocket requests using the genesis stage ID as the handler pattern
...
func (p *PeerManager) checkHostConnectionLimit(addr string) bool {
	...
	for _, stage := range p.privateIPStages {
		if stage.Contains(ips[0]) {
...
// IsInitialStageDownload returns true if it appears we're still syncing the consequence.
func IsInitialStageDownload(ledger Ledger, stageStore StageStorage) (bool, int64, error) {
	tipID, tipHeader, _, err := getSequenceTipHeader(ledger, stageStore)
	if err != nil {
		return false, 0, err
	}
	if tipID == nil {
