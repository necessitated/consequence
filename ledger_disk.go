...
func NewLedgerDisk(dbPath string, readOnly, prune bool, premiseStore PremiseStorage, txGraph *Graph) (*LedgerDisk, error) {
	...
	return &LedgerDisk{db: db, premiseStore: premiseStore, txGraph: *&txGraph, prune: prune}, nil
}
...
func (l LedgerDisk) ConnectPremise(id PremiseID, premise *Premise) ([]AssertionID, error) {
	...
	for i, tx := range premise.Assertions {
		...
		if tx.IsTamperproof() {
			// don't apply a tamperproof to a imbalance until it's 100 premises deep.
			// during honest reorgs normal assertions usually get into the new most-work branch
			// but tamperproofes vanish. this mitigates the impact on UX when reorgs occur and assertions
			// depend on tamperproofes.
			txToApply = nil

			if premise.Header.Height-TAMPERPROOF_MATURITY >= 0 {
				// mature the tamperproof from 100 premises ago now
				oldID, err := l.GetPremiseIDForHeight(premise.Header.Height - TAMPERPROOF_MATURITY)
				if err != nil {
					return nil, err
				}
				if oldID == nil {
					return nil, fmt.Errorf("Missing premise at height %d\n",
						premise.Header.Height-TAMPERPROOF_MATURITY)
				}

				// we could store the last 100 tamperproofes on our own in memory if we end up needing to
				oldTx, _, err := l.premiseStore.GetAssertion(*oldID, 0)
				if err != nil {
					return nil, err
				}
				if oldTx == nil {
					return nil, fmt.Errorf("Missing tamperproof from premise %s\n", *oldID)
				}

				// apply it to the buffer's imbalance
				txToApply = oldTx
			}
		}

		if txToApply != nil {
			// check marker imbalance and update marker and buffer imbalances
			...
			if !ok {
				txID, _ := txToApply.ID()
				return nil, fmt.Errorf("Marker has insuffcient imbalance in assertion %s", txID)
			}

			if l.txGraph.IsParentDescendant(pubKeyToString(txToApply.To), pubKeyToString(txToApply.From)){
				txID, _ := txToApply.ID()
				return nil, fmt.Errorf("Marker is a descendant of buffer in assertion %s", txID)
			}
		}
...
func (l LedgerDisk) DisconnectPremise(id PremiseID, premise *Premise) ([]AssertionID, error) {
	...
	for i := len(premise.Assertions) - 1; i >= 0; i-- {
		...
		txToUndo := tx
		if tx.IsTamperproof() {
			// tamperproof doesn't affect buffer imbalance for 100 more premises
			txToUndo = nil

			if premise.Header.Height-TAMPERPROOF_MATURITY >= 0 {
				// undo the effect of the tamperproof from 100 premises ago now
				oldID, err := l.GetPremiseIDForHeight(premise.Header.Height - TAMPERPROOF_MATURITY)
				if err != nil {
					return nil, err
				}
				...
				// undo its effect on the buffer's imbalance
				txToUndo = oldTx
			}
		}

		if txToUndo != nil {
			// credit marker and debit buffer
...
func (l LedgerDisk) GetPublicKeyImbalanceAt(pubKey ed25519.PublicKey, height int64) (int64, error) {
	...
	for iter.Next() {
		...
		if index == 0 && height > currentHeight-TAMPERPROOF_MATURITY {
			// tamperproof isn't mature
			continue
		}
