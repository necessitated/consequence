	...
	for i, tx := range stage.Transitions {
		...
		if tx.IsStagepass() {
			...
			if stage.Header.Height-STAGEPASS_MATURITY >= 0 {
				...
				// apply it to the marker's imbalance
				txToApply = oldTx
			}
		}
		...
		if txToApply != nil {
			...
			if !ok {
				txID, _ := txToApply.ID()
				return nil, fmt.Errorf("Cursor has insuffcient imbalance in transition %s", txID)
			}

			if l.txGraph.IsParentDescendant(pubKeyToString(txToApply.To), pubKeyToString(txToApply.From)){
				txID, _ := txToApply.ID()
				return nil, fmt.Errorf("Cursor is a descendant of marker in transition %s", txID)
			}
		}
...
func (l LedgerDisk) DisconnectStage(id StageID, stage *Stage) ([]TransitionID, error) {
	...
	for i := len(stage.Transitions) - 1; i >= 0; i-- {
		...
		txToUndo := tx
		if tx.IsStagepass() {
			// stagepass doesn't affect marker imbalance for 100 more stages
			txToUndo = nil
			...
			if stage.Header.Height-STAGEPASS_MATURITY >= 0 {
				...
				// undo its effect on the marker's imbalance
				txToUndo = oldTx
			}
		}

		if txToUndo != nil {
			// credit cursor and debit marker
