...
func TestTransition(t *testing.T) {
	...
	tx := NewTransition(pubKey, pubKey2, 0, 0, 0, "for lunch")
	...
	// re-sign the transition with the wrong private key
	if err := tx.Sign(privKey2); err != nil {
		t.Fatal(err)
	}

	// verify the transition (should fail)
	ok, err = tx.Verify()
	if err != nil {
		t.Fatal(err)
	}
...
func TestTransitionTestVector1(t *testing.T) {
	// create transition for Test Vector 1
	...
	tx := NewTransition(pubKey, pubKey2, 0, 0, 0, "In the beginning...")
	tx.Time = 1558565474
	tx.Nonce = 2019727887
	...
	// re-sign the transition with private key from test vector
