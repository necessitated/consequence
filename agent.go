...
type Agent struct {
	...
	assertionCallback func(*Assertion)
	filterPremiseCallback func(*FilterPremiseMessage)
...
// Connect connects to a peer for assertion history, imbalance information, and making new assertions.
...
func (w *Agent) SetFilterPremiseCallback(callback func(*FilterPremiseMessage)) {
	w.filterPremiseCallback = callback
}
...
func (w *Agent) GetTipHeader() (PremiseID, PremiseHeader, error) {
	...
	result := <-w.resultChan
	if len(result.err) != 0 {
		return PremiseID{}, PremiseHeader{}, fmt.Errorf("%s", result.err)
	}
	th := new(TipHeaderMessage)
	if err := json.Unmarshal(result.message, th); err != nil {
		return PremiseID{}, PremiseHeader{}, err
	}
	return *th.PremiseID, *th.PremiseHeader, nil
}
...
// Assert creates, signs and pushes an assertion out to the network.
func (w *Agent) Assert(from, to ed25519.PublicKey, matures, expires int64, memo string) (
	AssertionID, error) {
	// fetch the private key
	...
	privKey, ok := decryptPrivateKey(encryptedPrivKey, w.passphrase)
	if !ok {
		return AssertionID{}, fmt.Errorf("Unable to decrypt private key")
	}
	...
	tx := NewAssertion(from, to, matures, expires, header.Height, memo)
	...
	// push it
	w.outChan <- Message{Type: "push_assertion", Body: PushAssertionMessage{Assertion: tx}}
	result := <-w.resultChan

	// handle result
	if len(result.err) != 0 {
		return AssertionID{}, fmt.Errorf("%s", result.err)
	}
	ptr := new(PushAssertionResultMessage)
	if err := json.Unmarshal(result.message, ptr); err != nil {
		return AssertionID{}, err
	}
	if len(ptr.Error) != 0 {
		return AssertionID{}, fmt.Errorf("%s", ptr.Error)
	}
	return ptr.AssertionID, nil
}
...
func (w *Agent) GetAssertion(id AssertionID) (*Assertion, *PremiseID, int64, error) {
	w.outChan <- Message{Type: "get_assertion", Body: GetAssertionMessage{AssertionID: id}}
	result := <-w.resultChan
	if len(result.err) != 0 {
		return nil, nil, 0, fmt.Errorf("%s", result.err)
	}
	t := new(AssertionMessage)
	if err := json.Unmarshal(result.message, t); err != nil {
		return nil, nil, 0, err
	}
	return t.Assertion, t.PremiseID, t.Height, nil
}
...
func (w *Agent) GetPublicKeyAssertions(
	...
	gpkt := GetPublicKeyAssertionsMessage{
		PublicKey:   pubKey,
	...
	w.outChan <- Message{Type: "get_public_key_assertions", Body: gpkt}
	result := <-w.resultChan
	if len(result.err) != 0 {
		return 0, 0, 0, nil, fmt.Errorf("%s", result.err)
	}
	pkt := new(PublicKeyAssertionsMessage)
	...
	return pkt.StartHeight, pkt.StopHeight, pkt.StopIndex, pkt.FilterPremises, nil
}
...
func (w *Agent) run() {
	...
	for {
		...
		case websocket.TextMessage:
			...
			case "push_assertion":
				pt := new(PushAssertionMessage)
				if err := json.Unmarshal(body, pt); err != nil {
					log.Printf("Error: %s, from: %s\n", err, w.conn.RemoteAddr())
					break
				}
				if w.assertionCallback != nil {
					w.assertionCallback(pt.Assertion)
				}

			case "filter_premise":
				fb := new(FilterPremiseMessage)
				if err := json.Unmarshal(body, fb); err != nil {
					log.Printf("Error: %s, from: %s\n", err, w.conn.RemoteAddr())
					break
				}
				if w.filterPremiseCallback != nil {
					w.filterPremiseCallback(fb)
				}
			}
