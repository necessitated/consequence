...
proofs     | Show immature tamper proof recognitions for all public keys
assert     | Make an assertion from one of your public keys to another
show       | Show new incoming assertions
txstatus   | Show confirmed assertion information given a assertion ID
...
Once the agentdb is initialized, you'll want to generate keys to make assertions on the consequence network. This can be achieved with the `genkeys` command and entering the count of keys to generate (1 or more):
