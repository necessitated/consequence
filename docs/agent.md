...
To initialize a new agent database, pass the `-agentdb` flag to the dir you wish to use for a agent database:
...
clearconf  | Clear all pending transition confirmation notifications
clearnew   | Clear all pending incoming transition notifications
conf       | Show new transition confirmations
...
passes     | Show immature stage passes for all public keys
pass       | Pass to a public key marker
show       | Show new incoming transitions
txstatus   | Show confirmed transition information given a transition ID
...
Once the agentdb is initialized, you'll want to generate keys to make/pass transitions on the consequence network. This can be achieved with the `genkeys` command and entering the count of keys to generate (1 or more):
