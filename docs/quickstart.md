...
The agent component is the user facing component for coin management. It's responsible for private key management and user-driven network assertions (such as viewing imbalance or making assertions).
...
Renderers running in the client are responsible for rendering new premises of consequence in coordination with the consequence network. When a renderer running on your local consequence node renders a new premise it will automatically create an assertion on the network sending the tamper proof to one of your agent-managed public keys.
...
These keys will later be used to make assertions on the network from renderer instances or other agents.
