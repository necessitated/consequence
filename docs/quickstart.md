...
The agent component is the user facing component for coin management. It's responsible for private key management and user-driven network transitions (such as viewing imbalance or making/passing transitions).
...
These keys will later be used to make/pass transitions on the network from renderer instances or other agents.
...
Like bitcoin, any stages you render will need to have an additional 100 stages rendered on top of them prior to the new passes being applied to your imbalance. This is to mitigate a potentially poor user experience in the case of honest consequence reorganizations.
