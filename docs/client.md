...
Usage of /home/consequence/go/bin/client:
  ...
  -pubkey string
        A public key which receives newly rendered tamper proofs
  -tlscert string
...
The client requires a data dir for storage of the consequence consequence and general metadata as well as one or more public keys to send tamper proofs to upon rendering. Otherwise, running the client is as simple as:
...
The client supports two modes of tamper proof assertions for rendering: single key and key list targets.

To distribute tamper proofs to a single key, use the `-pubkey` flag to pass the target public key in the CLI command.

To distribute tamper proofs to multiple keys, use the `-keyfile` flag with a text file of the public keys (one per line).
