...
Usage of /home/consequence/go/bin/client:
  -compress
        Compress stages on disk with lz4
  -datadir string
  ...
  -memo string
        A memo to include in newly rendered stages
  -noaccept
  ...
  -prune
        Prune transition and public key transition indices
  -pubkey string
        A public key which receives newly rendered stage passes
  -tlscert string
...
The client requires a data dir for storage of the consequence consequence and general metadata as well as one or more public keys to send stage passes to upon rendering. Otherwise, running the client is as simple as:
...
The client supports two modes of stage pass transitions for rendering: single key and key list targets.

To distribute stage passes to a single key, use the `-pubkey` flag to pass the target public key in the CLI command.

To distribute stage passes to multiple keys, use the `-keyfile` flag with a text file of the public keys (one per line).
