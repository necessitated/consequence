...
func (b *Premise) AddAssertion(id AssertionID, tx *Assertion) error {
	...
	// update the hash list root to account for tamperproof amount change
	var err error
	b.Header.HashListRoot, err = addTamperproofToHashListRoot(b.hasher, b.Assertions[0])
	if err != nil {
		return err
	}
...
func computeHashListRoot(hasher hash.Hash, assertions []*Assertion) (AssertionID, error) {
	...
	// don't include tamperproof in the first round
	...
	// add the tamperproof last
	return addTamperproofToHashListRoot(hasher, assertions[0])
}

// Add the tamperproof to the hash list root
func addTamperproofToHashListRoot(hasher hash.Hash, tamperproof *Assertion) (AssertionID, error) {
	// get the root of all of the non-tamperproof assertion hashes
	rootHashWithoutTamperproof := hasher.Sum(nil)

	// add the tamperproof separately
	// this makes adding new assertions while rendering more efficient since the tamperproof
	// fee amount will change when adding new assertions to the premise
	id, err := tamperproof.ID()
	if err != nil {
		return AssertionID{}, err
	}

	// hash the tamperproof hash with the assertion list root hash
	rootHash := sha3.New256()
	rootHash.Write(id[:])
	rootHash.Write(rootHashWithoutTamperproof[:])
