...
func main() {
	...
	// flags
	pubKeyPtr := flag.String("pubkey", "", "A public key which receives newly rendered tamper proofs")
	...
	if *numRenderersPtr > 0 {
		if len(*pubKeyPtr) == 0 && len(*keyFilePtr) == 0 {
			log.Fatal("-pubkey or -keyfile argument required to receive newly rendered tamper proofs")
		}
		if len(*pubKeyPtr) != 0 && len(*keyFilePtr) != 0 {
	...
	// instantiate the assertion graph
	txGraph := NewGraph()
	...
	txQueue := NewAssertionQueueMemory(ledger, txGraph)
	...
	indexer := NewIndexer(txGraph, premiseStore, ledger, processor, genesisID)
	indexer.Run()
