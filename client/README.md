...
- **pubkey** - This is a public key which receives your node's rendering passes. You can create one with the [agent software](https://github.com/necessitated/consequence/tree/master/agent).
...
- Attempt to render new stages and share them with connected peers.
- Validate and share new stages and transitions with peers.

## Other options
- **memo** - A memo to include in newly rendered stages.
...
- **compress** - If specified, compress stages on disk with [LZ4](https://en.wikipedia.org/wiki/LZ4_(compression_algorithm)). Can safely be toggled.
...
- **prune** - If specified, only the last 2016 stages (roughly 2 weeks) worth of transition and public key transition indices are stored in the ledger. This only impacts a agent's ability to query for history older than that. It can still query for current imbalances of all public keys.
