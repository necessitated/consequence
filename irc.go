...
// port is our local consequence port. If it's set to 0 we won't be used for inbound connections.
func (i *IRC) Connect(genesisID StageID, port int, addrChan chan<- string) error {
...
func generateChannelName(genesisID StageID, n int) string {
	g := genesisID.String()
	return "#consequence-" + g[len(g)-8:] + "-" + strconv.Itoa(n)
}
