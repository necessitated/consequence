...
// StageHeaderHasher is used to more efficiently hash JSON serialized stage headers while rendering.
type StageHeaderHasher struct {
	// these can change per attempt
	previousHashListRoot     TransitionID
	previousTime             int64
	previousNonce            int64
	previousTransitionCount int32

	// used for tracking offsets of mutable fields in the buffer
	hashListRootOffset     int
	timeOffset             int
	nonceOffset            int
	transitionCountOffset int
...
var hdrTransitionCount []byte = []byte(`,"transition_count":`)
var hdrEnd []byte = []byte("}")

// NewStageHeaderHasher returns a newly initialized StageHeaderHasher
func NewStageHeaderHasher() *StageHeaderHasher {
	// calculate the maximum buffer length needed
	bufLen := len(hdrPrevious) + len(hdrHashListRoot) + len(hdrTime) + len(hdrTarget)
	bufLen += len(hdrSequenceWork) + len(hdrNonce) + len(hdrHeight) + len(hdrTransitionCount)
	bufLen += len(hdrEnd) + 4*64 + 3*19 + 10

	// initialize the hasher
	return &StageHeaderHasher{
...
func (h *StageHeaderHasher) initBuffer(header *StageHeader) {
	...
	// transition_count
	h.previousTransitionCount = header.TransitionCount
	buffer = append(buffer, hdrTransitionCount...)
	bufLen += len(hdrTransitionCount)
	h.transitionCountOffset = bufLen
	buffer = strconv.AppendInt(buffer, int64(header.TransitionCount), 10)
...
func (h *StageHeaderHasher) Update(rendererNum int, header *StageHeader) (*big.Int, int64) {
	if !h.initialized {
		h.initBuffer(header)
	} else {
		// hash_list_root
		...
		if offset != 0 || (h.previousNonce != header.Nonce) {
			...
			if offset != 0 {
				...
				// start of transition_count
				buffer = append(buffer, hdrTransitionCount...)
			}
		}

		// transition_count
		if offset != 0 || h.previousTransitionCount != header.TransitionCount {
			h.previousTransitionCount = header.TransitionCount

			// write out the new value (or old value at a new location)
			h.transitionCountOffset += offset
			bufLen := h.transitionCountOffset
			buffer := strconv.AppendInt(h.buffer[:bufLen], int64(header.TransitionCount), 10)
