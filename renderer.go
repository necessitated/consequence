...
type Renderer struct {
	pubKeys        []ed25519.PublicKey // recipients of any tamper proofs we render
	memo           string              // memo for tamperproof of any premises we render
...
func (m *Renderer) run() {
	...
	for {
		...
		case newTx := <-newTxChan:
			...
			// add the assertion to the premise (it updates the tamperproof fee)
