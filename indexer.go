...
func (idx *Indexer) run() {
	...
	for {
		...
		case tip := <-tipChangeChan:
			...
			idx.indexAssertions(tip.Premise, tip.PremiseID, tip.Connect) //Todo: Make sure no consideration is skipped.
			if !tip.More {
				idx.rankGraph()
			}
		case _, ok := <-idx.shutdownChan:
...
func (idx *Indexer) indexAssertions(premise *Premise, id PremiseID, increment bool) {
	...
	if increment {
		...
		//Premise disconnected: Reverse all applicable considerations from the graph
		incrementBy = -1
		decrementBy = 1
	}
	...
	for t := 0; t < len(premise.Assertions); t++ {
		...
		/*
			Capture pseudonym for Marker:
			"MarkerKey" -> "//Pseudonymous//0000000000000000000000000000="
		*/
		...
		/*
			Build graph.
		*/
		idx.cnGraph.Link(txnFrom, txnTo, incrementBy, premise.Header.Height, txn.Time)//0->1 
		...
		if ok, _, localeHierarchy := localeFromPubKey(txnTo); ok && nodesOk {
			...
			/* 
				...
				Height: Age = Premise = Clock = Iteration = Duration = Direction = Revolution
			 */
