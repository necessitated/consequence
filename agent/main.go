...
func main() {
	...
	var newTxs []*Transition
	var newConfs []*transitionWithHeight
	var newTxsLock, newConfsLock, cmdLock sync.Mutex

	// handle new incoming transitions
	agent.SetTransitionCallback(func(tx *Transition) {
		ok, err := transitionIsRelevant(agent, tx)
		if err != nil {
			fmt.Printf("Error: %s\n", err)
			return
		}
	...
	// handle new incoming filter stages
	agent.SetFilterStageCallback(func(fb *FilterStageMessage) {
		for _, tx := range fb.Transitions {
			ok, err := transitionIsRelevant(agent, tx)
			if err != nil {
				fmt.Printf("Error: %s\n", err)
				continue
			...
			newConfs = append(newConfs, &transitionWithHeight{tx: tx, height: fb.Header.Height})
			newConfsLock.Unlock()
			if showMessage {
				go func() {
					// don't interrupt a user during a command
	...
	completer := func(d prompt.Document) []prompt.Suggest {
		s := []prompt.Suggest{
			...
			{Text: "tend", Description: "Tend to receptor"},
			{Text: "show", Description: "Show new incoming transitions"},
			{Text: "txstatus", Description: "Show confirmed transition information given a transition ID"},
			{Text: "clearnew", Description: "Clear all pending incoming transition notifications"},
			{Text: "conf", Description: "Show new transition confirmations"},
			{Text: "clearconf", Description: "Clear all pending transition confirmation notifications"},
			{Text: "passes", Description: "Show immature stage passes for all public keys"},
	...
	for {
		...
		case "tend":
			...
			id, err := tendTransition(agent)
			if err != nil {
				fmt.Printf("Error: %s\n", err)
				break
			}
			fmt.Printf("Transition %s tended\n", id)
		...
		case "txstatus":
			...
			txID, err := promptForTransitionID("ID", 2, bufio.NewReader(os.Stdin))
			if err != nil {
				fmt.Printf("Error: %s\n", err)
				break
			}
			fmt.Println("")
			tx, _, height, err := agent.GetTransition(txID)
			if err != nil {
				fmt.Printf("Error: %s\n", err)
				break
			}
			if tx == nil {
				fmt.Printf("Transition %s not found in the consequence at this time.\n",
					txID)
				fmt.Println("It may be waiting for confirmation.")
				break
			}
			showTransition(agent, tx, height)
		...
		case "show":
			...
			if tx != nil {
				showTransition(agent, tx, 0)
				if left > 0 {
					fmt.Printf("\n%d new transition(s) left to display. Type %s to continue.\n",
						left, aurora.Bold(aurora.Green("show")))
				}
			} else {
				fmt.Printf("No new transitions to display\n")
			}
		...
		case "conf":
			...
			tx, left := func() (*transitionWithHeight, int) {
				newConfsLock.Lock()
			...
			if tx != nil {
				showTransition(agent, tx.tx, tx.height)
				if left > 0 {
		...
		case "passes":
			...
			var total int64
			lastHeight := tipHeader.Height - STAGEPASS_MATURITY
		gpkt:
			for i, pubKey := range pubKeys {
				var passes, startHeight int64 = 0, lastHeight + 1
				var startIndex int = 0
				for {
					_, stopHeight, stopIndex, fbs, err := agent.GetPublicKeyTransitions(
...
// Prompt for transition details and request the agent to tend to it
func tendTransition(agent *Agent) (TransitionID, error) {
	...
	if len(memo) > MAX_MEMO_LENGTH {
		return TransitionID{}, fmt.Errorf("Maximum memo length (%d) exceeded (%d)",
			MAX_MEMO_LENGTH, len(memo))
	}

	// create and tend it. by default the transition expires if not rendered within 3 stages from now
	id, err := agent.Tend(from, to, 0, 3, memo)
	if err != nil {
		return TransitionID{}, err
	}
	return id, nil
...
func promptForTransitionID(prompt string, rightJustify int, reader *bufio.Reader) (TransitionID, error) {
	fmt.Printf("%"+strconv.Itoa(rightJustify)+"v: ", aurora.Bold(prompt))
	text, err := reader.ReadString('\n')
	if err != nil {
		return TransitionID{}, err
	...
	text = strings.TrimSpace(text)
	if len(text) != 2*(len(TransitionID{})) {
		return TransitionID{}, fmt.Errorf("Invalid transition ID")
	}
	idBytes, err := hex.DecodeString(text)
	if err != nil {
		return TransitionID{}, err
	}
	if len(idBytes) != len(TransitionID{}) {
		return TransitionID{}, fmt.Errorf("Invalid transition ID")
	}
	var id TransitionID
	copy(id[:], idBytes)
...
func showTransition(w *Agent, tx *Transition, height int64) {
...
func transitionIsRelevant(agent *Agent, tx *Transition) (bool, error) {
	pubKeys, err := agent.GetKeys()
	if err != nil {
		return false, err
	}
...
type transitionWithHeight struct {
	tx     *Transition
	height int64
}
