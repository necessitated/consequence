...
func main() {
	...
	completer := func(d prompt.Document) []prompt.Suggest {
		s := []prompt.Suggest{
			...
			{Text: "assert", Description: "Make an assertion from one of your public keys to another"},
			{Text: "show", Description: "Show new incoming assertions"},
			{Text: "txstatus", Description: "Show confirmed assertion information given a assertion ID"},
			...
			{Text: "proofs", Description: "Show immature tamper proof recognitions for all public keys"},
	...
	for {
		...
		case "assert":
			...
			id, err := makeAssertion(agent)
			if err != nil {
				fmt.Printf("Error: %s\n", err)
				break
			}
			fmt.Printf("Assertion %s made\n", id)
		...
		case "proofs":
			...
			var total int64
			lastHeight := tipHeader.Height - TAMPERPROOF_MATURITY
		gpkt:
			for i, pubKey := range pubKeys {
				var proofs, startHeight int64 = 0, lastHeight + 1
...
// Prompt for assertion details and request the agent to execute to it
func makeAssertion(agent *Agent) (AssertionID, error) {
	...
	// create and assert it. by default the assertion expires if not rendered within 3 premises from now
	id, err := agent.Assert(from, to, 0, 3, memo)
	if err != nil {
		return AssertionID{}, err
	}
	return id, nil
