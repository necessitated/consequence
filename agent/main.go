...
func main() {
	...
	completer := func(d prompt.Document) []prompt.Suggest {
		s := []prompt.Suggest{
			...
			{Text: "pass", Description: "Pass to a marker"},
	...
	for {
		...
		case "pass":
			...
			id, err := passTransition(agent)
			if err != nil {
				fmt.Printf("Error: %s\n", err)
				break
			}
			fmt.Printf("Transition %s passed\n", id)
...
// Prompt for transition details and request the agent to pass to it
func passTransition(agent *Agent) (TransitionID, error) {
	...
	// create and pass it. by default the transition expires if not rendered within 3 stages from now
	id, err := agent.Pass(from, to, 0, 3, memo)
	if err != nil {
		return TransitionID{}, err
	}
	return id, nil
