...
A agent is a lightweight client which connects to a peer to receive imbalance and transition history information.
It also stores and manages private keys on behalf of the user and can be used to sign and publish transitions with them.
...
- **peer** - Specifies the address of a peer to talk to for imbalance and transition history information. It will also publish newly signed transitions to this peer. By default, it connects to `127.0.0.1:8832`.
...
You should only connect the agent to a client peer you trust. A bad client can misbehave in all sorts of ways that could confuse your agent and trick you into making/passing transitions you otherwise wouldn't. You also expose which public keys you control to the peer.
