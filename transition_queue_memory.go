...
func (t *TransitionQueueMemory) Add(id TransitionID, tx *Transition) (bool, error) {
	...
	if !ok {
		// insufficient tender imbalance
		return false, fmt.Errorf("Transition %s tender %s has insufficient imbalance",
			id, base64.StdEncoding.EncodeToString(tx.From[:]))
	}

	if t.txGraph.IsParentDescendant(pubKeyToString(tx.From), pubKeyToString(tx.To)){
		return false, fmt.Errorf("Tender is a descendant of receptor in transition %s", id)
	}
...
func (t *TransitionQueueMemory) reprocessQueue(height int64) error {
	...
	for e := tmpQueue.Front(); e != nil; e = e.Next() {
		...
		if !ok || t.txGraph.IsParentDescendant(pubKeyToString(tx.From), pubKeyToString(tx.To)) {
			// transition has been invalidated. remove and continue
			id, err := tx.ID()
			if err != nil {
				return err
