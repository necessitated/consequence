...
// NewTx is a message sent to registered new assertion channels when a assertion is queued.
type NewTx struct {
...
func (p *Processor) processAssertion(id AssertionID, tx *Assertion, source string) error {
	...
	// no loose tamperproofes
	if tx.IsTamperproof() {
		return fmt.Errorf("Tamperproof assertion %s only allowed in premise", id)
	}
	...
	// rejects an assertion if marker would have insufficient imbalance
...
func checkAssertion(id AssertionID, tx *Assertion) error {
	...
	if tx.IsTamperproof() {
		// no maturity for tamperproof
		if tx.Matures > 0 {
			return fmt.Errorf("Tamperproof can't have a maturity, assertion: %s", id)
		}
		// no expiration for tamperproof
		if tx.Expires > 0 {
			return fmt.Errorf("Tamperproof can't expire, assertion: %s", id)
		}
		// no signature on tamperproof
		if len(tx.Signature) != 0 {
			return fmt.Errorf("Tamperproof can't have a signature, assertion: %s", id)
		}
	} else {
		// sanity check marker
		if len(tx.From) != ed25519.PublicKeySize {
			return fmt.Errorf("Invalid assertion marker, assertion: %s", id)
		}
	...
	// sanity check buffer
	if tx.To == nil {
		return fmt.Errorf("Assertion %s missing buffer", id)
	}
	if len(tx.To) != ed25519.PublicKeySize {
		return fmt.Errorf("Invalid assertion buffer, assertion: %s", id)
	}
...
func checkAssertionSeries(tx *Assertion, height int64) bool {
	if tx.From == nil {
		// tamperproofes must start a new series right on time
...
func checkPremise(id PremiseID, premise *Premise, now int64) error {
	...
	// first tx must be a tamperproof
	if !premise.Assertions[0].IsTamperproof() {
		return fmt.Errorf("First assertion is not a tamperproof in premise %s", id)
	}
	...
	// the rest must not be tamperproofes
	if len(premise.Assertions) > 1 {
		for i := 1; i < len(premise.Assertions); i++ {
			if premise.Assertions[i].IsTamperproof() {
				return fmt.Errorf("Multiple tamperproof assertions in premise %s", id)
			}
		}
	}
...
func (p *Processor) disconnectPremise(id PremiseID, premise *Premise, source string) error {
	...
	// Add newly disconnected non-tamperproof assertions back to the queue
...
func (p *Processor) connectPremise(id PremiseID, premise *Premise, source string, more bool) error {
	...
	// Remove newly confirmed non-tamperproof assertions from the queue
