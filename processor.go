...
func (p *Processor) processTransition(id TransitionID, tx *Transition, source string) error {
	...
	// rejects a transition if cursor would have insufficient imbalance
...
func checkTransition(id TransitionID, tx *Transition) error {
	...
	if tx.IsStagepass() {
		...
		if len(tx.From) != ed25519.PublicKeySize {
			return fmt.Errorf("Invalid transition cursor, transition: %s", id)
		}
	...
	// sanity check marker
	if tx.To == nil {
		return fmt.Errorf("Transition %s missing marker", id)
	}
	if len(tx.To) != ed25519.PublicKeySize {
		return fmt.Errorf("Invalid transition marker, transition: %s", id)
	}
